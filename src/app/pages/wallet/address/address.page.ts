...
import { AddressBalance } from "src/app/models/blockbook/blockbook";
...
export class WalletAddressPage implements OnInit, OnDestroy {
    ...
    async init() {
        this.paramsSub = this.route.paramMap.subscribe(async (params) => {
            ...
            this.AddressInfo = await this.getAddressesBalance();
            this.onGoingProcesService.clear();
        });
    }

    async getAddressesBalance() {
        return await this.blockbookService.getAddressesBalance(this.credentials);
    }
}
